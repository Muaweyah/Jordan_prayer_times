...
data class MushafLine(val type: String, val text: String, val isCentered: Boolean)
...
class QuranActivity : AppCompatActivity() {
        ...
        Thread {
            ...
            for (page in 1..604) {
                try {
                    ...
                    // نتجاوز صفحة فشل تحميلها ونكمل الباقي
                }
            ...
            try {
                ...
                // فشل الحفظ لن يمنع الاستخدام، فقط سيُعاد البناء المرة القادمة
            }
            surahStartPage = map
    ...
    // ===== تحميل الصفحات من مصدر بيانات المصحف المفتوح =====
    ...
    private fun parsePage(pageNumber: Int, json: String): MushafPage {
        ...
        for (i in 0 until linesArray.length()) {
            ...
            when (lineObj.optString("type")) {
                "surah-header" -> lines.add(MushafLine("surah-header", lineObj.optString("text", ""), true))
                "basmala" -> lines.add(MushafLine("basmala", "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ", true))
                else -> {
                    lines.add(MushafLine("text", lineObj.optString("text", ""), false))
